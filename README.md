...
7. 合成链供给器只接受订单产生的资源，不能由管道写入；右击可随时打开配置界面，但只有先用网络联结器选择供给器并扫描目标机器后，界面才会列出可勾选的配方类型，不支持手工填写；若供给器总共只有一个候选且尚未配置，该候选会在绑定完成时自动选中并立即启用。一个配方类型包含多个逻辑材料组时，类型按钮下会出现较窄的材料组多选按钮：全部未选表示接受全部材料组，选中任意项后则只接受所选组；不同供给器可分别承接同一步骤的不同材料组。扫描成功后，供给器四个水平侧面中央会显示最近目标方块的 Jade 风格 8×8 对应物品图标。库存可由外部管道提取。
