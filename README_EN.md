...
- **Craftline Provisioners**: A provisioner accepts order-generated resources for selected recipe types and exposes its inventory for external extraction. The Network Linker can bind specific faces of remote devices for round-robin, nearest-first, or farthest-first direct delivery. Each provisioner accepts 16 devices by default (server-configurable), with connection state shown in its GUI, Jade, and world highlights.
...
7. Craftline Provisioners accept only order-produced resources and cannot be filled by pipes. Right-click one to open its configuration screen; left-clicking it with the linker opens the same GUI. After assigning recipe types, right-click the provisioner with the linker to enter wireless binding mode, then right-click the desired face of each device. Clicking another face updates that device; clicking its current face disconnects it. Connected and crosshair-previewed faces are highlighted in-world. A provisioner accepts 16 devices by default through the server `provisioner.maxWirelessConnections` setting. The button on the right of the connected-device row cycles through round-robin, nearest-first, and farthest-first delivery. A target that rejects the current resource is skipped so the next target is still attempted. The GUI's third line from the bottom and Jade show the connection count. Clearing recipe types also clears every device connection. Sneak-right-clicking the provisioner and scanning a target remains the recipe-candidate workflow; recipe groups, side target icons, external extraction, and “Return all to network” retain their previous behavior.
