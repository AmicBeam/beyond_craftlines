...
服务端规划具有可配置硬预算，默认单次最多展开 4096 个需求节点、递归深度最多 48，并在约 50 ms 后中止；同一配方内选为相同物品的输入先合并再递归。订单提交间隔、全服及单玩家活动订单上限同样可在 server 配置中调整，默认分别为 10 tick、1024 和 64。超过预算或限额时必须拒绝下单，不能占用服务端主线程继续展开。
...
终态订单历史默认只保留最近 200 条，并可配置为 0～100000；没有状态变化的等待 tick 不重复标记 SavedData。活动订单与有限历史共同构成持久化范围，避免世界运行时间越长，调度和存档成本越高。

普通合成按模拟批次限速。每到一个允许的执行时点，先用实际输入调用一次原配方；若输入和返还物的组件状态在批次间稳定，则按 BD 的 `long` 数量把当前网络中可用的同组件输入一次扣除，并把动态产物及容器返还物等比例写回，不按原版 64 堆叠或订单总量逐个循环。不同 Data Components 的输入分别成批，避免背包内容等状态被合并。

若某个输入会返还为同物品但组件发生变化，例如小刀损耗、灵魂图腾耐久或其他状态机工具，则不能安全地把一次结果乘以十亿；该节点继续逐次模拟，使上一轮返还的实际状态成为下一轮输入。完全不变的催化物可以跨整个批次复用一份。批次间隔由服务端配置 `crafting.virtualCraftingNodeIntervalTicks` 控制，默认 `20` tick。
