...
| 原版普通熔炉、高炉、烟熏炉能否替代 BD 网络炉？ | 普通机器绑定仍保留原始 JEI UID，本次修复不改变其语义，不能据此视为 BD 原生炉替代品。 |
...
普通绑定机器仍保存原始 JEI UID，本次修复不把它自动升级为原生炉替代品。
...
`0.4.0` 的供给器只保存原始 UID，不能满足第二项；`0.5.0` 会把供给器规范化为相同
执行族，因此可以通过第二项校验。普通绑定机器保持原始 UID 语义。
...
| 普通绑定机器执行族 | 保留原始 JEI UID，不作为原生炉替代端点 | 相同 |
...
| 7 | 绑定具备自动化能力的普通原版熔炉，在没有 BD 网络熔炉和供给器时提交炉类订单 | 不得把普通绑定视为 BD 原生执行端 | 相同 |
