...
final class MatchingStock<K, G>
{
                          ...
                          ToLongFunction<K> capacityPerItem, long fallbackCapacity)
    {
        ...
        if (amounts != null) for (var entry : amounts.entrySet())
        {
            ...
            long neededItems = remaining / capacity + (remaining % capacity == 0 ? 0 : 1);
            long usedItems = Math.min(entry.getValue().total(), neededItems);
            items = SaturatingLongMath.add(items, usedItems);
            long covered = SaturatingLongMath.multiply(usedItems, capacity);
            if (covered >= remaining) return items;
            remaining -= covered;
        }
        long capacity = Math.max(1, fallbackCapacity);
        long created = remaining / capacity + (remaining % capacity == 0 ? 0 : 1);
