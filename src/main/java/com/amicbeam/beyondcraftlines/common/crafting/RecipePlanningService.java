...
public final class RecipePlanningService
{
                                      ...
                                      PlanningBudget budget)
    {
        ...
        for (RecipeResourceResolver.ResourceIngredient ingredient : recipeIngredients)
            ingredient.candidates().stream().filter(candidate -> candidate.key() instanceof ItemStackKey)
                    .findFirst().ifPresent(candidate -> baselineSelections.add(
                            new RecipePlan.IngredientSelection(ingredient.slot(),
                                    IngredientSelectionKey.exact(candidate.key()))));
                                             ...
                                             List<KeyAmount> variant)
    {
        ...
        KeyAmount result = recipeOutputs.stream()
                .filter(value -> RecipeIoProfileRegistry.outputMatches(
                        holder.value(), holder.id().toString(), outputKey, value.key()))
                                                     ...
                                                     IStackKey<?> resource)
    {
        ...
        for (var entry : byOutput.entrySet())
        {
            List<RecipeHolder<?>> configured = entry.getValue().stream().filter(holder ->
                    RecipeIoProfileRegistry.outputMatches(holder.value(), holder.id().toString(),
