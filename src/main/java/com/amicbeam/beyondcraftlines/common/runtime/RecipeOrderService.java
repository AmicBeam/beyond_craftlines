...
public final class RecipeOrderService
{
        ...
        for (int pass = 0; pass < 2; pass++)
        {
            for (int step = 0; step < working.executions().size(); step++)
            {
                ...
                if (previousWait != null && currentWait == null)
                {
                    ...
                    inFlightOutputs.removeIf(output -> execution.step().matchRule()
                            .matches(execution.step().outputKey(), output));
                }
                if (currentWait != null)
    ...
    private record InputChunk(com.wintercogs.beyonddimensions.api.storage.key.IStackKey<?> key,
                              long amount, boolean fromReserved, String inputGroup,com.amicbeam.beyondcraftlines.common.crafting.ResourceMatchRule matchRule){private InputChunk(IStackKey<?> key,long amount,boolean reserved,String group){this(key,amount,reserved,group,com.amicbeam.beyondcraftlines.common.crafting.ResourceMatchRule.STRICT);}}
