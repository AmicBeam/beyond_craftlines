...
When a recipe has alternatives, right-click its product node to open or cycle the recipe choice. Hold **Ctrl** while right-clicking to remove the override and restore automatic selection.
...
Manual recipe and ingredient choices are saved as planner preferences and reused in later trees. A missing, unloaded, or no-longer-valid preference is ignored. The server still validates every selected recipe and ingredient when previewing and submitting the order.
