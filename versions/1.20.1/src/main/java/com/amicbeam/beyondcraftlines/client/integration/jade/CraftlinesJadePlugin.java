...
public final class CraftlinesJadePlugin implements IWailaPlugin {
    ...
    private enum Provider implements IBlockComponentProvider, IServerDataProvider<BlockAccessor> {
        ...
        @Override public void appendServerData(CompoundTag data, BlockAccessor accessor) {
            var server = accessor.getLevel().getServer(); if (server == null) return;
            ListTag types = new ListTag();
            BindingSavedData.get(server).recipeTypesForProvisioner(accessor.getLevel().dimension(), accessor.getPosition()).stream()
        ...
        @Override public void appendTooltip(ITooltip tooltip, BlockAccessor accessor, IPluginConfig config) {
            ...
            for (int i = 0; i < types.size(); i++) {
                ...
                if (title == null) title = Component.translatable("tooltip.jade.beyond_craftlines.provisioner_recipe.unknown");
                tooltip.add(Component.translatable("tooltip.jade.beyond_craftlines.provisioner_recipe.bound", title));
            }
        }
