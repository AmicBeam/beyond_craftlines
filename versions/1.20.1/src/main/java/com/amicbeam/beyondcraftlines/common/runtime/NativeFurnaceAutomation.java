...
public final class NativeFurnaceAutomation {
    ...
    public static long insertCapacity(BaseNetFurnaceBlockEntity<?> furnace, ResourceLocation itemId, long limit) { return insertCapacity(furnace, key(itemId), limit); }
    public static long insertCapacity(BaseNetFurnaceBlockEntity<?> furnace, ItemStackKey key, long limit) { return transferInto(furnace.getInputStorageSlots(), key, limit, true); }
    public static long insertCapacity(BaseNetFurnaceBlockEntity<?> furnace, IStackKey<?> key, long limit) { return key instanceof ItemStackKey item ? insertCapacity(furnace, item, limit) : 0; }
    public static long insert(BaseNetFurnaceBlockEntity<?> furnace, ResourceLocation itemId, long amount) { return insert(furnace, key(itemId), amount); }
    public static long insert(BaseNetFurnaceBlockEntity<?> furnace, ItemStackKey key, long amount) { return transferInto(furnace.getInputStorageSlots(), key, amount, false); }
    public static long insert(BaseNetFurnaceBlockEntity<?> furnace, IStackKey<?> key, long amount) { return key instanceof ItemStackKey item ? insert(furnace, item, amount) : 0; }
    public static boolean containsAnyInput(BaseNetFurnaceBlockEntity<?> furnace, Set<ResourceLocation> itemIds) {
    ...
    public static List<KeyAmount> extractOutputStacks(BaseNetFurnaceBlockEntity<?> furnace, ResourceLocation itemId, long amount) {
    ...
    public static long restoreOutput(BaseNetFurnaceBlockEntity<?> furnace, ResourceLocation itemId, long amount) { return restoreOutput(furnace, key(itemId), amount); }
    public static long restoreOutput(BaseNetFurnaceBlockEntity<?> furnace, ItemStackKey key, long amount) { return transferInto(furnace.getOutputStorageSlots(), key, amount, false); }
