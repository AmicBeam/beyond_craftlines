...
public record VirtualInputUse(Kind kind, int damagePerCraft)
{
    ...
    {
        if (kind == null || damagePerCraft < 0 || kind == Kind.DURABILITY && damagePerCraft < 1
    ...
    public long requiredAmount(long crafts, IStackKey<?> key, long amountPerCraft)
    {
        ...
        if (kind == Kind.CONSUMED) return SaturatingLongMath.multiply(crafts, amountPerCraft);
    ...
    long requiredAmount(long crafts,IStackKey<?> key,long amountPerCraft,
                        MatchingStock<IStackKey<?>,net.minecraft.resources.Identifier> stock)
    {
    ...
    public long requiredAmount(long crafts,IStackKey<?> key,long amountPerCraft,java.util.Map<IStackKey<?>,Long> stock)
    {
        ...
        return SaturatingLongMath.add(items,DurabilityInputMath.requiredTools(remaining,1,usesPerItem(key)));
    }
