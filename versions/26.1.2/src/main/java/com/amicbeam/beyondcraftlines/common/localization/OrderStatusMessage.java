...
public final class OrderStatusMessage
{
    ...
    private static Decoded decodeLegacy(String stored)
    {
        Decoded exact = switch (stored)
        {
            ...
            case "generic machine automation does not support output items that are also inputs" -> decoded("shared_input_output_unsupported");
            case "blocking mode: target machine still contains a recipe input" -> decoded("blocking_machine_input");
            case "bound machine reserved; preparing inputs" -> decoded("bound_machine_preparing");
            case "waiting for pre-existing bound machine byproducts to clear" -> decoded("bound_machine_byproducts_clear");
            case "BD network furnace is busy" -> decoded("native_furnace_busy");
            case "blocking mode: BD network furnace still contains a recipe input" -> decoded("blocking_native_furnace_input");
            case "waiting for pre-existing BD network furnace output to clear" -> decoded("native_furnace_output_clear");
            ...
            case "network has no room for crafting output or remaining items" -> decoded("crafting_network_full");
            case "crafting ingredients changed before execution" -> decoded("crafting_inputs_changed");
            case "crafting transaction rolled back because network capacity changed" -> decoded("crafting_rolled_back");
            default -> null;
        };
        if (exact != null) return exact;
        if (stored.startsWith("execution failed; waiting to return reserved materials:")) return decoded("execution_failed_returning");
        if (stored.startsWith("waiting to return reserved materials:")) return decoded("waiting_return_reserved");
        if (stored.startsWith("BD network furnace unavailable for ")) return decoded("native_furnace_unavailable", after(stored, "BD network furnace unavailable for "));
        if (stored.startsWith("bound machine unavailable for ")) return decoded("bound_machine_unavailable", after(stored, "bound machine unavailable for "));
        if (stored.startsWith("provisioner has no room for ")) return decoded("provisioner_no_room", after(stored, "provisioner has no room for "));
        if (stored.startsWith("provisioner inputs delivered; waiting for network output ")) return decodedProgress("provisioner_waiting_output", stored, "provisioner inputs delivered; waiting for network output ");
        if (stored.startsWith("machine processing; returned ")) return decodedProgress("machine_processing", stored, "machine processing; returned ");
        if (stored.startsWith("waiting for provisioner output in network ")) return decodedProgress("provisioner_waiting_output", stored, "waiting for provisioner output in network ");
        if (stored.startsWith("waiting for ")) return decoded("waiting_resource", after(stored, "waiting for "));
        if (stored.startsWith("BD network furnace processing; returned ")) return decodedProgress("native_furnace_processing", stored, "BD network furnace processing; returned ");
        if (stored.startsWith("crafting recipe is no longer available: ")) return decoded("crafting_recipe_unavailable", after(stored, "crafting recipe is no longer available: "));
        if (stored.startsWith("recipe simulation failed:")) return decoded("crafting_simulation_failed");
        return decoded("unknown");
    }

    private static String after(String value, String prefix) { return value.substring(prefix.length()); }
    ...
    private static Decoded decodedProgress(String id, String value, String prefix)
    {
        ...
        return slash < 0 ? decoded(id, progress, "?") : decoded(id, progress.substring(0, slash), progress.substring(slash + 1));
    }
    ...
    public record Decoded(String translationKey, List<String> arguments)
    {
        ...
        public Decoded { arguments = List.copyOf(arguments); }
