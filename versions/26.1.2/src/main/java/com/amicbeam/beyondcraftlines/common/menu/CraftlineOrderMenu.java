...
public final class CraftlineOrderMenu extends AbstractContainerMenu
{
    ...
    private final Map<Identifier, RecipeHolder<?>> recipeByOutput;
    private final Map<Identifier, List<RecipeHolder<?>>> recipesByOutput;
                              ...
                              Identifier initialRecipe, Set<String> availableFamilies)
    {
        ...
        this.recipes = RecipePlanningService.visibleRecipes(player.level()).stream()
                ...
                .toList();
        LinkedHashMap<Identifier, RecipeHolder<?>> byOutput = new LinkedHashMap<>();
        LinkedHashMap<Identifier, List<RecipeHolder<?>>> allByOutput = new LinkedHashMap<>();
        LinkedHashMap<IStackKey<?>, List<RecipeHolder<?>>> allByResourceOutput = new LinkedHashMap<>();
        for (RecipeHolder<?> holder : recipes)
        {
            for (var output : RecipeOutputResolver.outputs(holder.value(), player.level()))
            {
        ...
        LinkedHashMap<Identifier, List<RecipeHolder<?>>> frozen = new LinkedHashMap<>();
    ...
    public RecipeHolder<?> recipeForOutput(Identifier output) { return recipeByOutput.get(output); }
