...
public final class DashboardConfigMenu extends AbstractContainerMenu
{
                                ...
                                CraftlineDashboardBlockEntity dashboard)
    {
        ...
        this.target = ItemStackKey.EMPTY;
    ...
    {
        target = value == null ? ItemStackKey.EMPTY : value;
        ItemStack display = target instanceof ItemStackKey item
    ...
    @Override public boolean stillValid(Player value)
    {
        return value == player && value.blockPosition().distSqr(position) <= 64
                && value.level().getBlockEntity(position) instanceof CraftlineDashboardBlockEntity;
    }
    ...
    @Override public void clicked(int slotId, int button, ContainerInput clickType, Player player)
    {
        if (slotId == SAMPLE_SLOT)
        {
            ...
            setGhostTarget(carried.isEmpty() ? ItemStackKey.EMPTY
                    : new ItemStackKey(carried.copyWithCount(1)));
            persistGhost(player);
            return;
        }
    ...
    @Override public ItemStack quickMoveStack(Player player, int index)
    {
        ...
        if (!stack.isEmpty())
        {
            setGhostTarget(new ItemStackKey(stack.copyWithCount(1)));
            persistGhost(player);
        }
