...
public final class NativeFurnaceAutomation
{
    ...
    public static long insertCapacity(BaseNetFurnaceBlockEntity<?> furnace, Identifier itemId, long limit)
    ...
    public static long insert(BaseNetFurnaceBlockEntity<?> furnace, Identifier itemId, long amount)
    ...
    public static long countOutput(BaseNetFurnaceBlockEntity<?> furnace, Identifier itemId)
    {
        return count(furnace.getOutputStorageSlots(), itemId);
    }

    public static long extractOutput(BaseNetFurnaceBlockEntity<?> furnace, Identifier itemId, long amount)
    {
        if (amount <= 0) return 0;
    ...
    public static long restoreOutput(BaseNetFurnaceBlockEntity<?> furnace, Identifier itemId, long amount)
    ...
    private static long count(StackHandler storage, Identifier itemId)
    {
        long total = 0;
    ...
    private static boolean matches(KeyAmount value, Identifier itemId)
    {
        return value.key() instanceof ItemStackKey itemKey
                && BuiltInRegistries.ITEM.getKey(itemKey.getSource()).equals(itemId);
    }

    private static ItemStackKey key(Identifier itemId)
    {
        return new ItemStackKey(new ItemStack(BuiltInRegistries.ITEM.getValue(itemId)));
    }
}
